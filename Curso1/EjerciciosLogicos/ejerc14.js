...
const convertir = (grado = undefined,unidad = undefined) => {
    ...
    if((grado === undefined) && (unidad === undefined) ) return console.warn("No ingresaste un dato valido");
    if(typeof grado !== "number") return console.warn("No ingresaste un numero");
    if(typeof unidad !== "string") return console.warn(`El valor "${grado}" ingresado, NO es una cadena de texto`);

    if(unidad.length !== 1 || !/(C|F)/.test(unidad)) return console.warn("Valor de grado no reconocido");
